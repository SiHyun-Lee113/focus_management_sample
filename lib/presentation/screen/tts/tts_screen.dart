...
class TtsScreen extends StatefulWidget {
  ...
  const TtsScreen({super.key, required this.ttsService});
...
class _TtsScreenState extends State<TtsScreen> {
  ...
  Future<dynamic> _getEngines() async => await _ttsService.getEngines();

  Future<dynamic> _getVoices() async => await _ttsService.getVoices();

  Future<dynamic> _getLanguages() async => await _ttsService.getLanguages();

  Future<void> _getDefaultEngine() async {
    if (!isAndroid) return; // safety-check
    if (kDebugMode) debugPrint('_getDefaultEngine...');
    var e = await _ttsService.getDefaultEngine();
    if (e != null) {
  ...
  Future<void> _getDefaultVoice() async {
    ...
    if (isAndroid) {
      var defVoice = await _ttsService.getDefaultVoice();
  ...
  Future<void> _speak() async {
    ...
    if (_newVoiceText != null) {
      if (_newVoiceText!.isNotEmpty) {
        await _ttsService.speak(_newVoiceText!);
      }
    }
  }
  ...
  Future<void> _stop() async {
    var result = await _ttsService.stop();
    if (result == 1) setState(() => ttsState = TtsState.stopped);
  }

  Future<void> _pause() async {
    var result = await _ttsService.pause();
    ...
    return Row(
      ...
      children: [
        ElevatedButton(
          ...
          onPressed: () async {
            _inputLength = await _ttsService.getMaxSpeechInputLength();
            setState(() {});
          },
        ),
